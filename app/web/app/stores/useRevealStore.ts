...
type RevealActions = {
  ...
  exploreCreativeDirector: (force?: boolean) => Promise<void>;
  searchReferences: (query: string) => Promise<void>;
  exploreSearchReferences: () => Promise<void>;
  searchedReferences: Reference[];
  searchedReferencesQuery: string;
...
export type RevealStore =
  RevealState & RevealActions;

export const useRevealStore =
  create<RevealStore>((set, get) => ({
    ...initialRevealState,
    ...
    setScene: (sceneId) => {
      ...
      set((state) => ({
        ...
        history:
          state.history.includes(scene.id)
            ? state.history
            : [
                ...state.history,
                scene.id,
    ...
    enterLaboratory: () => {
      ...
      set((state) => ({
        scene: {
          activeSceneId:
            firstLaboratoryScene.id,

          sceneTitle:
            firstLaboratoryScene.title,

          sceneDescription:
            firstLaboratoryScene.description,
        },
    ...
    registerObservation: (
      ...
      const storedObservation:
        ...
        createdAt:
          createdObservation.createdAt ??
          now,

        updatedAt:
          createdObservation.updatedAt ??
          now,
      };
      ...
      const exploredReferenceIds =
        Array.from(
          new Set([
            ...
            ...state.project.dna.references.map(
              (reference) =>
                reference.id
            ),
          ])
        ...
        project: {
          ...
          dna: {
            ...
            creativeDirector: {
              ...
              proposalNumber:
                evaluation.proposalNumber,

              exploredReferenceIds:
                evaluation.exploredReferenceIds,
    ...
    exploreCreativeDirector: async (force = false) => {
      const state = get();
      ...
      const input = {
        intention: {
          whatToReveal:
            state.project.dna.identity.whatToReveal,

          whatToTransmit:
            state.project.dna.identity.whatToTransmit,

          context:
            state.project.dna.identity.context,
        },
      ...
      const discovered =
        (force || evaluation.decision === "continue")
          ? await discovery.discover(
              input,
              exploration,
              state.project.dna.creativeDirection,
              4
            )
          : [];
      ...
      set((state) => ({
        project: {
          ...
          dna: {
            ...
            creativeDirector: {
              ...
              exploredReferenceIds:
                nextExploredReferenceIds,

              proposedReferenceIds:
                nextProposedReferenceIds,
              ...
              criteria:
                evaluation.nextExploration
                  ? [
                      ...state.project.dna.creativeDirector.criteria,
                      evaluation.nextExploration,
                    ]
                  : state.project.dna.creativeDirector.criteria,

              decisions: [
                ...state.project.dna.creativeDirector.decisions,
                evaluation.reason,
              ],

              decisionHistory: [
                ...state.project.dna.creativeDirector.decisionHistory,
        ...
        discoveredReferences: [
          ...state.discoveredReferences,
          ...discovered.filter(
            (reference) =>
              !state.discoveredReferences.some(
                (existing) =>
    ...
    searchedReferences: [],
    searchedReferencesQuery: "",

    searchReferences: async (query) => {
      const state = get();
      ...
      set({
        searchedReferencesQuery: trimmedQuery,
      });
      ...
      set((state) => ({
        searchedReferences: [
          ...state.searchedReferences,
          ...results.filter(
            (reference) =>
              !state.searchedReferences.some(
                (existing) =>
                  existing.id === reference.id
              )
          ),
    ...
    exploreSearchReferences: async () => {
      const state = get();

      const query =
        state.searchedReferencesQuery.trim();
       ...
       * RƎVELA · EXPLORAR MÁS
       *
       * Las referencias que ya aparecieron
       * en la búsqueda quedan excluidas.
       *
       * El Provider ya implementa esta exclusión
       * mediante exploredReferenceIds.
       */

      const searchedIds =
        state.searchedReferences.map(
          (reference) =>
            reference.id
        );
      ...
      set((current) => ({
        searchedReferences: [
          ...current.searchedReferences,
          ...results.filter(
            (reference) =>
              !current.searchedReferences.some(
                (existing) =>
                  existing.id === reference.id
              )
          ),
