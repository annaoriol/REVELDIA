...
export default function CreativeDirectorScene() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      setExampleIndex(
        (current) =>
          (current + 1) % inspirationExamples.length
      );
    }, 4200);
   ...
   * FASE I
   *
   * RƎVELAR se activa únicamente cuando
   * las tres peticiones están completas:
   *
   * 1. Punto de partida
   * 2. Cuéntame algo más
   * 3. Formato
  ...
  const handleReveal = () => {
    ...
    const whatToTransmit = `Crear un ${selectedFormat!.toLowerCase()}`;
  ...
  return (
    <section className="relative overflow-hidden">
      <div className="relative z-10 flex min-h-[min(74dvh,860px)] flex-col justify-center px-[clamp(1.25rem,6vw,7rem)] py-[clamp(3rem,7vw,7rem)]">
            ...
            <div className="mb-10">
              ...
              <div className="relative border-b border-white/15 transition-colors duration-300 focus-within:border-[var(--revela-accent)]">
                <textarea
                  ...
                  rows={2}
                  placeholder={
                    inspirationExamples[exampleIndex]
                  }
